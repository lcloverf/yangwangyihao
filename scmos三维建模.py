...
mask= judge_outliers(6,mask)[0]
X=judge_outliers(6,mask)[1]
print('*'*30)
gauss_clustering_again(X,6,mask)

# mask1=gaussian_mixture(X,6)

# gmm = mixture.GaussianMixture(covariance_type='full', n_components=6)
# gmm.fit(X)
# clusters = gmm.predict(X)
# print(clusters[0])
# clusters = clusters.reshape(old_shape[2], old_shape[1])  #根据mask的index赋值，先循环，
...
#     # print(com3.mean(axis=0))#列
#
#     x= [0,4,8,12,16,20,24,28,32,36,40,44,48,52,56,60,64,68,72,76,80,84,88,92,96,100,104,108,112,116,120,124,128,132,136]
#     x = np.array(x)
#     # print('x is :\n', x)
#     y=com3.mean(axis=0)
#     y = np.array(y)
...
#         if B[h]>(3*mean):
#            print(xx[h],yy[h])
#            mask1[xx[h],yy[h]]=None
#            index = yy[h]*60 + xx[h]
#            X=np.delete(X,index,axis=0)
#     # mse_num = ((((yvals - com3)**2).sum()))**0.5/(com3.shape[0]*com3.shape[1])
#     # print("残差:",mse_num)
#     # MSE=MSE+mse_num
# print(mask1)
# print(MSE/6)
